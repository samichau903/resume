...
import { WORK_EXPERIENCES, MENTORSHIP_EXPERIENCE, PERSONAL_DETAILS } from "../data";
...
export default function ExperienceSection({ selectedSkill, onSkillClick }: ExperienceSectionProps) {
  return (
            ...
            <div className="space-y-4">
              ...
              {MENTORSHIP_EXPERIENCE.map((exp) => {
                ...
                return (
                   <div
                    key={exp.id}
                    className={`p-6 bg-white dark:bg-slate-950 border rounded-xl transition-all duration-300 ${
                      usesActiveSkill
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-2 mb-4">
                      ...
                      <div className="flex items-center gap-1.5 text-xs font-mono text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-950 border border-slate-200/60 dark:border-slate-850 px-2 py-1 rounded">
            ...
            <div className="space-y-8">
              ...
              {WORK_EXPERIENCES.map((exp) => {
                ...
                return (
                  <div
                    key={exp.id}
                    className={`p-6 bg-white dark:bg-slate-950 border rounded-xl transition-all duration-300 relative ${
                      usesActiveSkill
                  ...
                  >
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-y-2 mb-4">
                      ...
                      <div className="flex items-center gap-1.5 text-xs font-mono text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900 px-2.5 py-1 rounded border border-slate-200 dark:border-slate-800">
                        <Calendar className="w-3.5 h-3.5 text-indigo-500 dark:text-indigo-400" />
                        <span>{exp.period}</span>
                      </div>
                    </div>
                    ...
                    {exp.summaryBlock && (
                      <p className="text-xs font-mono italic text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900 p-3 rounded-lg border-l-2 border-indigo-500 mb-4">
                        {exp.summaryBlock}
                      </p>
                    )}
                    ...
                    {exp.metrics && (
                      <div className="grid grid-cols-3 gap-3 bg-slate-50 dark:bg-slate-900/60 p-3 rounded-lg border border-slate-150 dark:border-slate-800 mb-4">
                        {exp.metrics.map((metric) => (
                          <div key={metric.label} className="text-center">
                            ...
                            <div className="text-[8px] font-mono text-slate-400 dark:text-slate-500 leading-none mt-1 scale-95">{metric.desc}</div>
                          </div>
                        ))}
                      </div>
                    )}
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="p-6 bg-white dark:bg-slate-955 border border-slate-200 dark:border-slate-800 rounded-xl space-y-4 shadow-xs">
              <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-900 pb-3">
                ...
                <h3 className="font-display font-bold text-slate-800 dark:text-slate-300 text-xs tracking-wider uppercase">
                  EDUCATION
                </h3>
              </div>
              ...
              <div className="space-y-2">
                {PERSONAL_DETAILS.languages.map((lang) => (
                  <div key={lang} className="flex justify-between items-center text-xs font-mono">
                    <span className="text-slate-505 dark:text-slate-400">{lang.split(" ")[0]}</span>
